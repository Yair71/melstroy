...
// Camera spawns at your exact screenshot coordinates
// ============================================================
...
export function initCamera(scene, rendererDom) {
    ...
    if (DEBUG) {
        ...
        camera.position.set(CONFIG.cameraStartX, CONFIG.cameraStartY, CONFIG.cameraStartZ);
        fly.yaw   = CONFIG.cameraYaw * Math.PI / 180;   // 179.2° → radians
        fly.pitch = CONFIG.cameraPitch * Math.PI / 180;  // 0°
...
function setupFlyCam(canvas) {
    const onClick = () => { if (!fly.isLocked) canvas.requestPointerLock(); };
    canvas.addEventListener('click', onClick);

    const onLockChange = () => { fly.isLocked = (document.pointerLockElement === canvas); };
    ...
    const onKeyDown = (e) => {
        ...
        if (e.code === 'ShiftLeft' || e.code === 'ShiftRight') fly.speed = CONFIG.flySpeedFast;
        if (e.code === 'KeyK') dumpCoordinates();
    };
    const onKeyUp = (e) => {
        fly.keys[e.code] = false;
        if (e.code === 'ShiftLeft' || e.code === 'ShiftRight') fly.speed = CONFIG.flySpeed;
    };
...
function buildOverlayHTML() {
    ...
    let html = `
        <div style="color:#FFD700;font-weight:bold;">🔧 FLY CAMERA DEBUG</div>
        <div style="border-top:1px solid #333;margin:4px 0;"></div>
        <div><span style="color:#ff0">CLICK</span>=lock <span style="color:#ff0">WASD</span>=move <span style="color:#ff0">E/Space</span>=up <span style="color:#ff0">Q/Ctrl</span>=down</div>
        <div><span style="color:#ff0">Shift</span>=fast <span style="color:#ff0">Scroll</span>=speed <span style="color:#FF003C;font-weight:bold;">K</span>=SAVE COORDS</div>
        <div style="border-top:1px solid #333;margin:4px 0;"></div>
...
function dumpCoordinates() {
    ...
    console.log(`%c📌 SAVED #${savedCoords.length}: (${coord.x}, ${coord.y}, ${coord.z}) yaw=${(fly.yaw*180/Math.PI).toFixed(1)}° pitch=${(fly.pitch*180/Math.PI).toFixed(1)}°`,
...
export function updateCamera(deltaTime) {
    ...
    if (coordOverlay) coordOverlay.innerHTML = buildOverlayHTML();
}
...
export function cleanupCamera() {
    ...
    fly.handlers = [];
    if (coordOverlay && coordOverlay.parentElement) coordOverlay.parentElement.removeChild(coordOverlay);
}
