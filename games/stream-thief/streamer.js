...
export function switchModel(modelKey) {
    ...
    // Центрирование и посадка
    const box = new THREE.Box3().setFromObject(gltf.scene);
    const center = box.getCenter(new THREE.Vector3());
    
    gltf.scene.position.x -= center.x;
    gltf.scene.position.z -= center.z;
    gltf.scene.position.y += (0 - box.min.y); // Низ попки ровно на 0 относительно streamerGroup (которая на seatHeight)
...
export function updateStreamer(deltaTime) {
    ...
    if (cycleTimer <= 0) {
        ...
        cycleTimer = 3.0 + Math.random() * 4.0;
    }
}
