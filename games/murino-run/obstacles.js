...
export const activeCoins = []; // Массив для монет
...
export function spawnObstacle(zPos) {
  ...
  // Спавн монеток на других линиях
...
export function updateObstacles(playerGroup, deltaTime) {
  ...
  for (let i = activeObstacles.length - 1; i >= 0; i--) {
    ...
    if (obs.position.z > playerGroup.position.z + 2 && !obs.userData.passed) {
      ...
      gameState.score += 10; // Очки даем, а монеты теперь нужно собирать физически
    }
  ...
  for (let i = activeCoins.length - 1; i >= 0; i--) {
      ...
      coin.rotation.z += 5 * deltaTime; // Монетки крутятся
