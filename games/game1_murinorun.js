export function createGame(root, api) {
  ...
  const ROAD_LEN = 120;
  const ROAD_COUNT = 9;
  const ROAD_RECYCLE_OFFSET = 0.02;
  ...
  async function preloadAssets() {
    try {
      ...
      function extractAnim(gltf, label) {
        ...
        let fixed = fixAnimation(sourceClip, playerModel);
        if (!fixed) {
          logInfo('ℹ️ fallback raw clip: ' + label);
          fixed = sourceClip.clone();
        }
  ...
  function lookCameraAt(x, y, z, amount = 0.15) {
  ...
  function setupWorld() {
    ...
    fogEntity = new THREE.Mesh(
      ...
      new THREE.MeshBasicMaterial({ map: loadedTextures.fog, transparent: true, depthWrite: false })
    );
    ...
    const dust = new THREE.Points(
      dustGeo,
      new THREE.PointsMaterial({ color: 0x666666, size: 0.08 })
    );
    dust.name = 'dust';
    scene.add(dust);
  }
  ...
  function animate() {
    ...
    if (gameState === STATE.INTRO) {
      ...
      // --- ROAD RECYCLE (дальше за камерой, чтобы не было видно смену) ---
      let frontMostZ = Infinity;
      for (const r of roadMeshes) {
        frontMostZ = Math.min(frontMostZ, r.position.z);
      }

      const recycleThreshold = camera.position.z + ROAD_LEN * 1.35;

      for (const r of roadMeshes) {
        if (r.position.z > recycleThreshold) {
          r.position.z = frontMostZ - ROAD_LEN + ROAD_RECYCLE_OFFSET;
          frontMostZ = r.position.z;
        }
      }
      ...
      buildings.forEach(b => {
        if (b.position.z > camera.position.z + 10) b.position.z -= 200;
      });
      ...
      if (deathPhase === 'fall') {
        ...
        fogEntity.position.y += (3.5 - fogEntity.position.y) * 0.05;
        ...
        lookCameraAt(fogEntity.position.x, fogEntity.position.y + 0.8, fogEntity.position.z, 0.18);

        fogEntity.position.x += (camera.position.x - fogEntity.position.x) * 0.03;
        fogEntity.position.y += ((camera.position.y + 0.15) - fogEntity.position.y) * 0.04;

        let nextFogZ = fogEntity.position.z - Math.max(deathStartSpeed * 2.8, 0.9) * frame;
        if (nextFogZ < camera.position.z + 2.2) nextFogZ = camera.position.z + 2.2;
        fogEntity.position.z = nextFogZ;

        fogEntity.lookAt(camera.position);

        if (deathTimer >= DEATH.chaseTime || fogEntity.position.z <= camera.position.z + 2.25) {
          deathPhase = 'eat';
          deathTimer = 0;
        }
      }
        ...
        tempVecB.y += 0.10;
        tempVecB.z += 0.02;
        camera.position.lerp(tempVecB, 0.2);

        const lockedFogZ = camera.position.z + 1.65;
        fogEntity.position.x += (camera.position.x - fogEntity.position.x) * 0.08;
        fogEntity.position.y += (camera.position.y - fogEntity.position.y) * 0.08;
        fogEntity.position.z = lockedFogZ;
        fogEntity.scale.setScalar(1 + deathTimer * 2.6);
        fogEntity.lookAt(camera.position);

        lookCameraAt(fogEntity.position.x, fogEntity.position.y, fogEntity.position.z - 0.4, 0.2);
  ...
  function triggerDeath() {
    ...
    fogEntity.scale.set(1, 1, 1);
    fogEntity.position.set(playerGroup.position.x, 4, camera.position.z + 24);
    fogEntity.lookAt(camera.position);
  ...
  function resetGame() {
    ...
    roadMeshes.forEach((r, i) => {
      r.position.z = -i * ROAD_LEN;
    });
